...
function exactDeployment(deployment) {
  const entryPoint = deployment?.deployments?.filter((item) =>
    item?.standard === "org.ethereum.erc-4337.entry-point.v0.8"
      && item?.role === "account-abstraction-entry-point") ?? [];
  const exchange = deployment?.deployments?.filter((item) =>
    item?.standard === "org.emsenn.semiotic-exchange-chain.v1"
      && item?.role === "modeled-union-dimension-exchange") ?? [];
  const factory = deployment?.deployments?.filter((item) =>
    item?.standard === "org.emsenn.evm.sovereign-enterprise-account.v3"
      && item?.role === "sovereign-settlement-account-factory") ?? [];
  if (deployment?.profile !== "org.emsenn.evm.standards-deployment.v2"
      || deployment?.type !== "UnionEvmStandardsDeployment" || deployment?.chainId !== CHAIN
      || !/^[0-9a-f]{64}$/u.test(deployment?.genesisSha256 ?? "")
      || entryPoint.length !== 1 || exchange.length !== 1 || factory.length !== 1
      || !ADDRESS.test(entryPoint[0].address ?? "")
      || !ADDRESS.test(exchange[0].address ?? "") || !ADDRESS.test(factory[0].address ?? "")
      || !HASH.test(exchange[0].transactionHash ?? "")
      || !Number.isSafeInteger(exchange[0].blockNumber)) {
    throw new TypeError(`one verified canonical ${CHAIN} SuccessorDeployment is required`);
  }
  return Object.freeze({
    chain: CHAIN,
    genesisSha256: deployment.genesisSha256,
    entryPoint: entryPoint[0].address.toLowerCase(),
    exchange: exchange[0].address.toLowerCase(),
    deploymentBlock: exchange[0].blockNumber,
    deploymentTransaction: exchange[0].transactionHash.toLowerCase(),
    factory: factory[0].address.toLowerCase(),
  });
}
...
function exactAccount(binding, deployment) {
  ...
  if (binding?.profile !== "org.emsenn.evm.sovereign-enterprise-account.v3"
      ...
      || !HASH.test(binding?.enterprise?.enterpriseId ?? "")
      || binding?.chain?.chainId !== 5615611 || binding?.chain?.caip2 !== CHAIN
      || !ADDRESS.test(account ?? "") || binding?.account?.caip10?.toLowerCase() !== `${CHAIN}:${account}`
      || binding?.factory?.address?.toLowerCase() !== deployment.factory
      || !HASH.test(binding?.factory?.userSalt ?? "")
...
function exactEconomic(binding, deployment, account) {
  if (binding?.type !== "SuccessorX402Binding" || binding?.network !== CHAIN
      ...
      || !ADDRESS.test(binding?.asset ?? "") || binding?.payTo?.toLowerCase() !== account.address
      || !/^https:\/\/[^\s]+$/u.test(binding?.facilitatorUrl ?? "")
  ...
  return Object.freeze({
    network: CHAIN, exchange: deployment.exchange, asset: binding.asset.toLowerCase(),
    payTo: account.address, facilitatorUrl: binding.facilitatorUrl,
    assetName: binding.assetName, assetVersion: binding.assetVersion, amount: binding.amount,
  });
}
...
function attachedSignature(receipt, payload, controller) {
  ...
  if (receipt?.type !== "DetachedContentSigningReceipt" || receipt?.version !== 1
      || receipt?.disposition !== "signed" || receipt?.payload !== payload
      || receipt?.controller?.toLowerCase() !== controller.toLowerCase()
      || receipt?.verification?.disposition !== "verified"
      || !NI.test(receipt?.id ?? "") || typeof receipt?.algorithm !== "string"
      || receipt.algorithm === "" || typeof receipt?.keyId !== "string" || receipt.keyId === ""
      || !signaturePresent) {
...
export function projectSettlementSuccessorActivation({ deployment, accountBinding, x402Binding, agentCardTemplate, offerSigningReceipt } = {}) {
  ...
  const manifestPayload = { type: "CapabilityCellManifest", version: 3, cell: identity.id, sku: SKU,
    capability: LAW, controller: account.controller, faces: {
      ...
      x402: { resource: "https://bare-cedar-fog.561.group/x402/settlement-pulse/invoke", offer: "https://bare-cedar-fog.561.group/x402/settlement-pulse/offer", scheme: "exact", ...economic },
  ...
  const offerPayload = { kind: "org.emsenn.capability-offer.v3", cell: identity.id, sku: SKU, provider: URN,
    ...
    price: { protocol: "x402", scheme: "exact", network: CHAIN, asset: economic.asset,
      amount: economic.amount, payTo: economic.payTo, facilitator: economic.facilitatorUrl,
  ...
  const residualBase = { version: 1, chain: CHAIN, controller: account.controller, identity: identity.id,
    manifest: manifest.id, offer: offer.id };
  const observed = { type: "CapabilityCellProjectionResidual", ...residualBase, state: "not-live-observed",
    observations: [], networkCallsMadeByProjector: 0 };
  observed.id = contentAddress(observed);
  ...
  const outbox = [{ "@context": "https://www.w3.org/ns/activitystreams",
    id: `https://bare-cedar-fog.561.group/activities/settlement-pulse/activation/${contentAddress(activityObject).slice("ni:///sha-256;".length)}`,
    type: "Announce", actor: "https://bare-cedar-fog.561.group/actors/settlement-pulse",
    to: "https://www.w3.org/ns/activitystreams#Public", object: activityObject }];
  const outputs = { "content/agent-cards/settlement-triggered-activity-publication-cell.json": card,
    "content/capcell/identity.json": identity, "content/capcell/manifest.json": manifest,
    "content/capcell/observed-projections.json": observed, "content/offers/current.json": offer,
    "content/invocations/current.json": invocation, "content/health/current.json": health,
    "content/activitypub/outbox.json": outbox, "content/migrations/eip155-5615611.json": activation };
  ...
  return Object.freeze({ type: "SuccessorActivationProjection", version: 1,
    id: contentAddress(outputs), nodeId: NODE, outputs: Object.freeze(outputs) });
}
